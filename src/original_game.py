...
def print_game(left_array, right_array, player_att, player_def):
    ...
    #announcement = "Player" + player_att + ", Unit" + unit_att + "bla bla" + player_def + ", unit" + unit_def + ". Armor blocked dammage"
    #decision = "Player " + player_def + "turn, unit " + unit_turn + "turn. Choose target:"
...
class Player:
    ...
    def complete_list(self, lista):
        ...
        while i < len(alpos) and self.gold > 24:
            ...
            if self.gold >= lista[adaug].cost:                
                alpos[i] = copy.copy(lista[adaug])
                self.eroi.append(alpos[i])
...
if __name__ == '__main__':
    ...
    ##################################################### Cine incepe ############################################################################
    jucatori = [p1, p2]
    print_game(p1.eroi, p2.eroi, jucatori[0].name, jucatori[1].name)
